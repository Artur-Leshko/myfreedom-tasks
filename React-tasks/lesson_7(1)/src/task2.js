...
class State extends React.Component {
  constructor(props) {
    ...
    const initialStateObject = isPlainObject(this.props.initialState);
    const valueField = !this.props.initialState ? {} : (initialStateObject ? {...this.props.initialState} : this.props.initialState)
  ...
  render() {
    return this.props.children({
      ...
      */
      state: this.state,
      setState: (value) => { this.setState(!value ? {} : (isPlainObject(value) ? {...value} : value)) }
    });
  }
}
...
const SettingsForm = ({
  ...
  <Form>
    <Form.Group>
      <Form.Label>Нотификации включены</Form.Label>
      <Form.Check
